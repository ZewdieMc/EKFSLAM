...
class MapFeature:
    ...
    the feature pose using the robot sensors (:meth:`GetFeatures`), as well as for computing its:

        * observation(sensor) model (:meth:`hf`),
        * inverse observation(sensor) model (:meth:`g`)
...
if __name__ == '__main__':
    ...
    #jacobian of the inverse sensor model wrt the state vector
    a = Cartesian2DMapFeature().Jgx(xs0, M[0])
    b = Polar2DMapFeature().s2o(M[0])#cartesian to polar
    c = Polar2DMapFeature().o2s(M[0])#polar to cartesian
